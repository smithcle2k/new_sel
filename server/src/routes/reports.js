...
const COMPLIANCE_SQL = `
  SELECT c.checkin_date              AS date,
   ...
   WHERE c.classroom_id = ?
     AND c.school_id    = ?          -- tenant guard, belt and braces
 ...
 */
reportRouter.get('/classroom/:id/compliance', h(async (req, res) => {
  const classroom = await scopedClassroom(req.user, req.params.id);
  const { from, to } = rangeFrom(req.query);
  const rows = await all(COMPLIANCE_SQL, [classroom.id, req.user.school_id, from, to]);
  res.json({
    classroom,
    range: { from, to },
...
reportRouter.get('/classroom/:id/compliance.csv', h(async (req, res) => {
  const classroom = await scopedClassroom(req.user, req.params.id);
  const { from, to } = rangeFrom(req.query);
  const rows = await all(COMPLIANCE_SQL, [classroom.id, req.user.school_id, from, to]);
...
reportRouter.get('/classroom/:id/meadow', h(async (req, res) => {
  const classroom = await scopedClassroom(req.user, req.params.id);
  const { from, to } = rangeFrom(req.query);

  const [students, checkins] = await Promise.all([
    all(`SELECT id, first_name, last_initial, fur_color, hat
           FROM students WHERE classroom_id = ? AND archived = 0
           ORDER BY first_name`, [classroom.id]),
    all(`SELECT student_id, checkin_date, mood_score, bloom
           FROM checkins
          WHERE classroom_id = ? AND checkin_date BETWEEN ? AND ?
          ORDER BY checkin_date`, [classroom.id, from, to]),
  ]);
...
reportRouter.get('/student/:studentId/history', h(async (req, res) => {
