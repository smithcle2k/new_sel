 ...
 *
 * SQLite has no native RLS, so isolation is enforced here instead: no route
...
export function scopedClassroom(user, classroomId) {
  const row = db
    .prepare('SELECT * FROM classrooms WHERE id = ? AND school_id = ?')
    .get(classroomId, user.school_id);
...
export function scopedStudent(user, studentId) {
  const row = db
    .prepare('SELECT * FROM students WHERE id = ? AND school_id = ?')
    .get(studentId, user.school_id);
  if (!row) throw new HttpError(404, 'Student not found.');
  scopedClassroom(user, row.classroom_id);
  return row;
}

/** Every classroom the caller may list. */
export function visibleClassrooms(user) {
  if (user.role === 'admin') {
    return db
      .prepare(`SELECT c.*, u.full_name AS teacher_name,
  ...
  return db
    .prepare(`SELECT c.*, u.full_name AS teacher_name,
                ...
                FROM classrooms c
                JOIN users u ON u.id = c.teacher_id
               WHERE c.school_id = ? AND c.teacher_id = ?
               ORDER BY c.name`)
    .all(user.school_id, user.id);
}
